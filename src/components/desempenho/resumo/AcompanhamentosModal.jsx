    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
        <div className="bg-white p-4 rounded-xl border shadow-sm">
          ...
          <p className="text-2xl font-black text-slate-800">{checkpointResumo.total}</p>
        </div>

        <div className="bg-white p-4 rounded-xl border shadow-sm">
          <p className="text-sm text-gray-500 font-bold">Melhoraram KM/L</p>
          <p className="text-2xl font-black text-emerald-700">
            {checkpointResumo.melhoraramKml}
          </p>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border shadow-sm">
          ...
          <p className="text-2xl font-black text-emerald-700">
            {checkpointResumo.reduziramDesperdicio}
          </p>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border shadow-sm">
          ...
          <p className="text-2xl font-black text-rose-700">
            {checkpointResumo.aumentaramDesperdicio}
          </p>
        </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {resumoInstrutor.map((row) => (
                <tr key={row.id} className="hover:bg-blue-50/50 transition-colors">
                  ...
                  <td className="px-4 py-4">{row.total_acompanhamentos}</td>
                  <td className="px-4 py-4">{formatMinutes(row.total_minutos)}</td>
                  <td className="px-4 py-4">{formatMinutes(row.media_minutos)}</td>
                  <td className="px-4 py-4">{row.em_monitoramento}</td>
                  <td className="px-4 py-4">{row.em_analise}</td>
                  <td className="px-4 py-4">{row.concluidos}</td>
                  <td className="px-4 py-4">{row.qtd_linhas}</td>
                </tr>
              ))}
            ...
            <tbody className="divide-y divide-gray-100">
              {resumoPorLinhaCheckpoint.map((row) => (
                <tr key={row.id} className="hover:bg-blue-50/50 transition-colors">
                  ...
                  <td className="px-4 py-4">{row.qtd_motoristas}</td>
                  ...
                  <td className="px-4 py-4 text-emerald-700 font-bold">{row.melhorou}</td>
                  <td className="px-4 py-4 text-rose-700 font-bold">{row.piorou}</td>
                  <td className="px-4 py-4 text-slate-700 font-bold">{row.sem_evolucao}</td>
                </tr>
              ))}
          ...
          <table className="w-full text-left min-w-[1700px]">
            ...
            <tbody className="divide-y divide-gray-100">
              {acompanhamentosComEvolucao.map((row) => (
                <tr key={row.id} className="hover:bg-blue-50/50 transition-colors">
                  <td className="px-4 py-4 font-black text-slate-900">{row.motorista_nome}</td>
                  <td className="px-4 py-4">{row.motorista_chapa}</td>
