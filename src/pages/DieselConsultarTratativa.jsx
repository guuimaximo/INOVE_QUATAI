...
// Consultar Diesel (módulo separado)
// - Busca:
//   diesel_tratativas
...
export default function DieselConsultarTratativa() {
        ...
        <ul className="space-y-1">
          {arr.map((u, i) => (
            <li key={`${u}-${i}`} className="text-sm">
              <a href={u} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">
                {fileNameFromUrl(u)}
              </a>
            </li>
          ))}
  ...
  useEffect(() => {
    (async () => {
      ...
      // Histórico Diesel (mais recente primeiro)
      const h = await supabase
        .from("diesel_tratativas_detalhes")
        .select("id, created_at, tratativa_id, acao_aplicada, observacoes, imagem_tratativa, anexo_tratativa, tratado_por_login, tratado_por_nome, tratado_por_id")
        .eq("tratativa_id", id)
  ...
  return (
    <div className="mx-auto max-w-5xl p-6">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold">Consultar (Diesel)</h1>
        <button
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
        ...
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            {renderListaArquivosCompacta(evidenciasSolicitacao, "Evidências da solicitação")}
          </div>
        </dl>
      </div>
