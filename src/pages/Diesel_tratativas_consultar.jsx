...
function Thumb({ url }) {
  ...
  return img ? (
    <a href={url} target="_blank" rel="noopener noreferrer" title="Abrir">
      <img src={url} alt={fileNameFromUrl(url)} className="h-14 w-14 rounded border object-cover hover:opacity-90" loading="lazy" />
    </a>
  ) : (
...
export default function Diesel_tratativas_consultar({ aberto, grupo, onClose }) {
        ...
        <div className="p-4 border-b flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-gray-500">
              ...
              {" "}• Arquivo: <span className="font-semibold">{grupo.arquivo_key}</span> • {grupo.qtd_atas} ata(s)
            </div>
          </div>
        ...
        <div className="p-4 space-y-4">
          <div className="bg-white shadow rounded-lg overflow-x-auto">
            <table className="min-w-full">
              ...
              <tbody>
                {(grupo.itens || []).map((i) => (
                  <tr key={`${i.ata_id}-${i.detalhe_id}`} className="border-t hover:bg-gray-50">
