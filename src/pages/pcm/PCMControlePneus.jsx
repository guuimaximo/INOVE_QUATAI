...
const ABAS_CONFERENCIA = ["veiculos", "estoque", "recapadora", "sucata"];
...
// embutido: vira o grupo "Conferência TransNet" da página de Controle de Pneus.
// A aba vem de fora (`aba`); `onResumo` recebe os números da conferência para a
// aba Resumo (por isso esta tela fica carregada, escondida, atrás do Resumo).
export default function PCMControlePneus({ embutido = false, aba: abaExterna, onResumo } = {}) {
  const filtrosIniciais = lerFiltrosSalvos();
  const [abaInterna, setAbaAtiva] = useState(
    ABAS_CONFERENCIA.includes(filtrosIniciais.abaAtiva) ? filtrosIniciais.abaAtiva : "veiculos"
  );
  const abaAtiva = embutido ? (ABAS_CONFERENCIA.includes(abaExterna) ? abaExterna : "veiculos") : abaInterna;
  ...
  }, [abaInterna, abaEstoqueAtiva, busca, buscaFogo, filtroStatus, filtroComparacao, somenteSelecionados, prefixosSelecionados, filtroDataAuditoria, ordenacaoTabela]);
  ...
  // Números da conferência para a aba Resumo (sempre sem filtro).
  const resumoConferencia = useMemo(() => {
    ...
    for (const row of linhasBase) {
      ...
      for (const posicao of POSICOES) {
        const cel = row.posicoes[posicao];
        if (cel?.aud) auditado = true;
        const status = cel?.status;
        if (!status) continue;
        totalPosicoes += 1;
        statusPosicoes[status] = (statusPosicoes[status] || 0) + 1;
        if (status !== "OK") divergencias.push({ posicao, status, detalhe: cel.detalheStatus || "" });
      }
      if (!auditado) semAuditoria += 1;
      if (divergencias.length) {
        comDivergencia.push({ prefixo: row.prefixo, ficha: row.ficha, auditoria_em: row.auditoria_em, divergencias });
      }
    }
    comDivergencia.sort((a, b) => b.divergencias.length - a.divergencias.length || String(a.prefixo).localeCompare(String(b.prefixo), "pt-BR", { numeric: true }));
    const porSituacao = {};
    for (const row of estoqueComparado.fisico) {
      const situacao = row.situacao_inove || "SEM SITUACAO";
      porSituacao[situacao] = (porSituacao[situacao] || 0) + 1;
    }
    const ultimaFisica = estoqueComparado.fisico.reduce((max, row) => (toTime(row.data_estoque) > toTime(max) ? row.data_estoque : max), null);
    return {
      snapshot: ultimaAtualizacao,
      veiculos: linhasBase.length,
      totalPosicoes,
      okPosicoes: statusPosicoes.OK || 0,
      statusPosicoes,
      comDivergencia,
      semAuditoria,
      estoque: {
        ...estoqueKpis,
        ficha: estoqueComparado.fisico[0]?.ficha_estoque || "",
        data: ultimaFisica,
        porSituacao,
      },
      recapadora: recapadoraKpis,
      sucata: sucataKpis,
    };
  }, [carregando, linhasBase, estoqueComparado, estoqueKpis, recapadoraKpis, sucataKpis, ultimaAtualizacao]);

  useEffect(() => {
    onResumo?.(resumoConferencia);
  }, [resumoConferencia, onResumo]);
  ...
  return (
    <div className={embutido ? "" : "min-h-[calc(100vh-64px)] bg-slate-100 p-3"}>
      <header className="mb-3 flex flex-wrap items-center gap-3">
        ...
        {/* Embutida, a data do TransNet e o "Atualizar" ficam no topo da página-mãe. */}
        {!embutido && (
          <>
        ...
        <button
          onClick={dispararBot}
          disabled={disparando}
          className={`${embutido ? "ml-auto " : ""}inline-flex items-center gap-2 rounded-lg bg-orange-600 px-3 py-1.5 text-sm text-white hover:bg-orange-700 disabled:opacity-50`}
        >
