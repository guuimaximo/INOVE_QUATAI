...
function getCategoriaStyle(cat) {
  ...
  return found || { value: cat, label: cat, color: "border-slate-300 bg-slate-50 text-slate-800", badge: "bg-slate-500 text-white" };
}
...
function AgingBar({ buckets }) {
  const total = (buckets["0-1"] || 0) + (buckets["2-3"] || 0) + (buckets["4-7"] || 0) + (buckets["8-15"] || 0) + (buckets["16+"] || 0);
  ...
  return (
    <div className="w-full">
      ...
      <div className="mt-4 grid grid-cols-2 md:grid-cols-5 gap-3">
        {parts.map((p) => (
          <div key={p.k} className="bg-slate-50 border border-slate-200 rounded-xl p-3 flex flex-col items-center justify-center shadow-sm">
            <div className="text-[10px] font-black text-slate-500 uppercase tracking-wider mb-1">{p.label} dias</div>
...
function Section({ title, subtitle, open, onToggle, children, right }) {
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden transition-all">
      <button type="button" onClick={onToggle} className="w-full px-5 py-4 flex items-center justify-between gap-3 hover:bg-slate-50 transition-colors">
        <div className="min-w-0 text-left">
          <div className="text-xs font-black text-slate-500 uppercase tracking-wider flex items-center gap-2">{title}</div>
          {subtitle && <div className="text-sm font-bold text-slate-800 truncate mt-1">{subtitle}</div>}
        </div>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-200">
        ...
        <div className="overflow-y-auto pr-2 space-y-5 text-sm text-slate-700">
          ...
          <div className="bg-blue-50 p-4 rounded-xl border border-blue-200">
            <h3 className="font-black text-blue-900 mb-2 flex items-center gap-2"><FaCheckDouble className="text-blue-600"/> Taxa de Resolução</h3>
        ...
        <div className="mt-5 pt-4 border-t flex justify-end shrink-0">
          <button onClick={onClose} className="px-6 py-2.5 bg-blue-600 text-white font-black rounded-xl hover:bg-blue-700 transition shadow-md active:scale-95">Entendi</button>
        </div>
      </div>
    </div>
  );
...
function FrotaDetalheModal({ open, onClose, frota, rows, periodo }) {
  ...
  const ciclos = sorted.map((r) => {
    ...
    const dias = dtIn ? daysDiff(dtIn, dtOut ? dtOut : new Date().toISOString()) : 0;
    return { ...r, _dias: dias };
  });
      ...
      <div className="bg-white w-full max-w-5xl max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="flex justify-between items-center p-5 border-b bg-white shrink-0 relative overflow-hidden">
          ...
          <div className="relative flex items-center gap-3">
            <div className="bg-blue-600 text-white p-2.5 rounded-lg shadow-sm"><FaCar size={20} /></div>
            <div>
              <h2 className="text-xl font-black text-slate-800">Detalhe da Frota <span className="text-blue-600">{frota || "-"}</span></h2>
              <div className="text-xs text-slate-500 font-bold mt-1">Ciclos e reincidências no período: {fmtBRDate(periodo?.inicio)} → {fmtBRDate(periodo?.fim)}</div>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-slate-50/50 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <CardKPI title="Registros" value={fmtInt(sorted.length)} sub="No período" icon={<FaTools />} tone="blue" />
            ...
            <div className="overflow-x-auto rounded-xl border border-slate-200">
              <table className="w-full text-left text-sm border-collapse min-w-[900px]">
                <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase tracking-wider text-[10px]">
                  <tr>
                    <th className="p-3 font-black">Data Ref</th><th className="p-3 font-black">Entrada</th><th className="p-3 font-black">Saída</th>
                    <th className="p-3 text-center font-black">Dias</th><th className="p-3 font-black">Categoria</th><th className="p-3 font-black">Setor</th>
                    <th className="p-3 font-black">O.S</th><th className="p-3 font-black w-[400px]">Descrição</th><th className="p-3 font-black">Observação</th>
                  </tr>
                </thead>
                <tbody>
                  {ciclos.map((r) => (
                    <tr key={r.id} className="border-b last:border-0 hover:bg-slate-50">
                      ...
                      <td className="p-3"><span className={`px-2 py-1 rounded text-[10px] font-black ${getCategoriaStyle(r.categoria).badge}`}>{r.categoria || "-"}</span></td>
                  ...
                  {ciclos.length === 0 && <tr><td colSpan={9} className="p-6 text-center text-slate-500 font-bold">Sem registros para esta frota.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
...
export default function PCMResumo() {
  ...
  const carregarSerieMensal = useCallback(async () => {
    const { data: pcms, error: e1 } = await supabase.from("pcm_diario").select("data_referencia").order("data_referencia", { ascending: true });
    if (e1) throw e1;
    ...
    const { data: veics, error: e2 } = await supabase.from("veiculos_pcm").select("categoria, data_entrada, pcm_id").eq("categoria", "GNS");
    if (e2) throw e2;
    ...
    if (needsPcm) {
      const pcmIds = Array.from(new Set((veics || []).map((v) => v.pcm_id).filter(Boolean)));
      const chunkSize = 500;
      for (let i = 0; i < pcmIds.length; i += chunkSize) {
        const chunk = pcmIds.slice(i, i + chunkSize);
        const { data: pcmChunk, error: e3 } = await supabase.from("pcm_diario").select("id, data_referencia").in("id", chunk);
        if (e3) throw e3;
    ...
    (veics || []).forEach((v) => {
      const dt = v.data_entrada ? toISODate(new Date(v.data_entrada)) : pcmMap.get(v.pcm_id);
  ...
  const carregarPeriodo = useCallback(async () => {
    ...
    const [qA, qB, qBacklog] = await Promise.all([
      ...
      supabase.from("veiculos_pcm").select("id, frota, categoria, observacao, data_entrada, pcm_diario(data_referencia)").is("data_saida", null)
    ]);
    ...
    const normalized = all.map((v) => ({ ...v, data_referencia: pcmIdToRef.get(v.pcm_id) || null }));
    const normalizedFiltered = dayFilter === "ALL" ? normalized : normalized.filter((v) => dayTypeFromISO(bestBaseDateISO(v)) === dayFilter);
    ...
    byFrota.forEach((arr, frota) => {
      ...
      reinc.push({ frota, entradas: sorted.length, reentradas, ultimaEntrada, diasParadoTotal, descricoesPreview: descricoes.slice(0, 3) });
    });
  ...
  const { kpis, analiseTatica } = useMemo(() => {
    ...
    veiculosPeriodo.forEach(v => {
      ...
      if (def.length > 25) def = def.substring(0, 25) + "...";
      if (def) defMap[def] = (defMap[def] || 0) + 1;

      if (v.lancado_no_turno === "DIA") turnoDia++;
      if (v.lancado_no_turno === "NOITE") turnoNoite++;
    });
    const topDefeitos = Object.entries(defMap).sort((a,b) => b[1]-a[1]).slice(0,6).map(([nome, val]) => ({nome, val}));
    ...
    const setores = Object.entries(bySetor).map(([k, v]) => ({ setor: k, total: v, pct: totalPeriodo ? safeNum((v / totalPeriodo) * 100, 1) : 0 })).sort((a, b) => b.total - a.total);
    const categorias = Object.entries(byCategoria).map(([k, v]) => ({ categoria: k, total: v, pct: totalPeriodo ? safeNum((v / totalPeriodo) * 100, 1) : 0 })).sort((a, b) => b.total - a.total);

    return {
      kpis: { diasComPCM, totalPeriodo, totalAbertos, totalGNS, pctGNS, mediaGNSDia, frotasReincidentes, aging: buckets, setores, categorias, taxaResolucao },
      analiseTatica: { motivosBacklog, topDefeitos, turnoDia, turnoNoite }
    };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 md:p-5">
        <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4">
          ...
          <div className="grid grid-cols-1 gap-2 sm:flex sm:flex-wrap">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl bg-blue-100 text-blue-800 font-bold hover:bg-blue-200 transition"
            >
            ...
            <button
              ...
              className={`inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl border text-xs font-black transition ${
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl border border-slate-800 bg-slate-800 text-white font-black hover:bg-slate-700 transition"
            >
              <FaRedo /> Atualizar
            </button>
          </div>
        ...
        <div className="mt-4 border-t pt-4 flex flex-col gap-3 md:flex-row md:items-center">
          ...
          <div className="flex flex-wrap gap-2">
            {DAY_FILTERS.map((p) => (
              <button key={p.key} onClick={() => setDayFilter(p.key)} className={`px-3 py-1.5 rounded-lg text-[11px] font-black uppercase tracking-wider transition ${dayFilter === p.key ? "bg-blue-600 text-white shadow" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                {p.label}
              </button>
            ))}
          </div>
        </div>
        {errMsg && <div className="mt-4 bg-rose-50 border border-rose-200 text-rose-700 rounded-xl p-3 text-sm font-bold flex items-center gap-2"><FaExclamationTriangle /> {errMsg}</div>}
      </div>

      {/* KPIs PRINCIPAIS */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-6 gap-4">
        <CardKPI title="Dias com PCM" value={fmtInt(kpis.diasComPCM)} sub="No período" icon={<FaCalendarAlt />} tone="slate" />
        <CardKPI title="Total Lançamentos" value={fmtInt(kpis.totalPeriodo)} sub="Veículos no período" icon={<FaBus />} tone="blue" />
        <CardKPI title="Taxa Resolução" value={fmtPct(kpis.taxaResolucao)} sub="Liberados no período" icon={<FaCheckDouble />} tone="indigo" />
        <CardKPI title="Total GNS" value={fmtInt(kpis.totalGNS)} sub={`${kpis.pctGNS}% do período`} icon={<FaChartPie />} tone="rose" />
        <CardKPI title="Média GNS/Dia" value={kpis.mediaGNSDia} sub="Gravidade Diária" icon={<FaChartLine />} tone="violet" />
        <CardKPI title="Reincidentes" value={fmtInt(kpis.frotasReincidentes)} sub="Mesmo carro quebrou 2x" icon={<FaRedo />} tone="amber" />
      </div>

      {/* AGING E GARGALOS (VISÃO BACKLOG REAL DE HOJE) */}
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        
        {/* AGING BAR */}
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm xl:col-span-2 flex flex-col justify-center">
        ...
        <div className="bg-slate-800 text-white border border-slate-700 rounded-2xl p-5 shadow-md flex flex-col">
          ...
          <div className="space-y-3 flex-1">
            {analiseTatica.motivosBacklog.map((m, i) => (
              <div key={i} className="flex justify-between items-center bg-slate-700/50 px-4 py-2.5 rounded-xl border border-slate-600">
                <span className="text-xs font-bold text-slate-200">{m.nome}</span>
                <span className="text-sm font-black text-white bg-slate-600 px-3 py-1 rounded-lg">{m.val}</span>
              </div>
            ))}
      ...
      {/* PAINEL DE INTELIGÊNCIA OPERACIONAL (Baseado no Período) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Top Sintomas/Defeitos */}
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm md:col-span-2">
          <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider mb-4 flex items-center gap-2">
            <FaClipboardList className="text-blue-500" /> Termômetro de Defeitos (Top 6 do Período)
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {analiseTatica.topDefeitos.map((d, i) => (
              <div key={i} className="flex justify-between items-center bg-slate-50 px-4 py-2.5 rounded-xl border border-slate-100">
                <span className="text-xs font-bold text-slate-700 truncate w-4/5" title={d.nome}>{d.nome}</span>
                <span className="text-sm font-black text-blue-600 bg-blue-100 px-3 py-1 rounded-lg">{d.val}</span>
              </div>
            ))}
            {analiseTatica.topDefeitos.length === 0 && <div className="text-xs font-bold text-slate-400">Nenhum defeito relatado no período.</div>}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
          <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider mb-4 flex items-center gap-2">
            <FaClock className="text-emerald-500" /> Entradas por Turno
          </h3>
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-blue-50 border border-blue-100 rounded-xl">
              <div className="flex items-center gap-3">
                <div className="bg-blue-600 text-white p-2 rounded-lg"><FaSun /></div>
                <div>
                  <div className="text-[10px] font-black text-blue-800 uppercase">Turno Dia</div>
                  <div className="text-2xl font-black text-blue-900">{analiseTatica.turnoDia}</div>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-indigo-50 border border-indigo-100 rounded-xl">
              <div className="flex items-center gap-3">
                <div className="bg-indigo-700 text-white p-2 rounded-lg"><FaMoon /></div>
                <div>
                  <div className="text-[10px] font-black text-indigo-800 uppercase">Turno Noite</div>
                  <div className="text-2xl font-black text-indigo-900">{analiseTatica.turnoNoite}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-4">
          <div>
            <h3 className="text-lg font-black text-slate-800">Frotas Reincidentes no Período</h3>
            <p className="text-xs text-slate-500 font-semibold">Clique no veículo para ver os detalhes da quebra</p>
          </div>
          ...
          <table className="w-full text-left text-sm border-collapse min-w-[700px]">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase tracking-wider text-[10px]">
              <tr>
                ...
                <th className="p-3 font-black w-1/3">Preview Histórico</th>
              </tr>
            </thead>
            <tbody>
