...
function TesteModal({ open, item, onClose, onSaved, user }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      const uploaded = await uploadSuprimentosFiles(newFiles, `testes/${form.prefixo || "sem-prefixo"}`);
      const payload = {
        ...
        <form onSubmit={handleSubmit} className="p-5">
          <div className="flex items-start justify-between gap-4 border-b border-slate-100 pb-4">
            <div>
              ...
              <h2 className="mt-3 text-2xl font-black text-slate-900">{item ? "Atualizar teste" : "Novo teste"}</h2>
              <p className="mt-2 text-sm font-medium text-slate-500">Acompanhe a peça em campo, a quilometragem do teste e o parecer final sem depender de mudança manual de status.</p>
            </div>
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <Field label="Nome do teste" required>
              <input value={form.nome_teste} onChange={(e) => setForm((prev) => ({ ...prev, nome_teste: e.target.value }))} className={inputClass} required />
            </Field>
            ...
            <Field label="Data de início" required>
              <input type="date" value={form.data_inicio} onChange={(e) => setForm((prev) => ({ ...prev, data_inicio: e.target.value }))} className={inputClass} required />
            </Field>
            <Field label="KM inicial" required>
              <input type="number" min="0" step="1" value={form.km_inicial} onChange={(e) => setForm((prev) => ({ ...prev, km_inicial: e.target.value }))} className={inputClass} required />
            </Field>
            <Field label="KM atual">
              <input type="number" min="0" step="1" value={form.km_atual} onChange={(e) => setForm((prev) => ({ ...prev, km_atual: e.target.value }))} className={inputClass} />
            </Field>
            <Field label="Prazo do teste">
              <input type="date" value={form.prazo_teste} onChange={(e) => setForm((prev) => ({ ...prev, prazo_teste: e.target.value }))} className={inputClass} />
            </Field>
            <Field label="Resultado final">
              <select value={form.resultado_final} onChange={(e) => setForm((prev) => ({ ...prev, resultado_final: e.target.value }))} className={inputClass}>
            ...
            <Field label="Data de encerramento">
              <input type="date" value={form.encerrado_em} onChange={(e) => setForm((prev) => ({ ...prev, encerrado_em: e.target.value }))} className={inputClass} />
            </Field>
            <label className="flex items-center gap-3 rounded-[24px] border border-slate-200 bg-slate-50 px-4 py-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm font-semibold text-slate-700">Marque quando a peça apresentou comportamento fora do esperado.</p>
              </div>
            </label>
          </div>
          ...
          {form.falha_registrada ? (
            <div className="mt-4 grid gap-4 md:grid-cols-3">
              <Field label="Data da falha">
                <input type="date" value={form.data_falha} onChange={(e) => setForm((prev) => ({ ...prev, data_falha: e.target.value }))} className={inputClass} />
              </Field>
              ...
              <Field label="Descrição da falha" className="md:col-span-3">
                <textarea value={form.descricao_falha} onChange={(e) => setForm((prev) => ({ ...prev, descricao_falha: e.target.value }))} className={`${inputClass} min-h-[110px]`} />
              </Field>
            </div>
          ) : null}
          ...
          <div className="mt-4 grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
            <Field label="Objetivo do teste">
              <textarea value={form.objetivo} onChange={(e) => setForm((prev) => ({ ...prev, objetivo: e.target.value }))} className={`${inputClass} min-h-[120px]`} />
            </Field>
            <Field label="Observação">
              <textarea value={form.observacao} onChange={(e) => setForm((prev) => ({ ...prev, observacao: e.target.value }))} className={`${inputClass} min-h-[120px]`} />
            </Field>
            <Field label="Parecer técnico" className="xl:col-span-2">
              <textarea value={form.parecer_tecnico} onChange={(e) => setForm((prev) => ({ ...prev, parecer_tecnico: e.target.value }))} className={`${inputClass} min-h-[120px]`} />
            </Field>
          </div>

          <div className="mt-4">
            <Panel title="Anexos" subtitle="Fotos, laudos, PDFs ou evidências do teste em campo.">
              <AttachmentInput
    ...
    >
      <div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Detail label="Início" value={formatDateBR(item.data_inicio)} />
          ...
          <Detail label="Falha" value={item.falha_registrada ? "Sim" : "Não"} />
          <Detail label="KM da falha" value={formatKm(item.km_falha)} />
        </div>

        <div className="space-y-4">
          <Block title="Objetivo" text={item.objetivo} />
          <Block title="Observação" text={item.observacao} />
          <Block title="Parecer técnico" text={item.parecer_tecnico} />
          {item.falha_registrada ? <Block title="Descrição da falha" text={item.descricao_falha} /> : null}
...
function Block({ title, text }) {
  return (
    <div className="rounded-[22px] border border-slate-200 bg-slate-50 p-4">
      ...
      <p className="mt-3 whitespace-pre-wrap text-sm font-medium leading-6 text-slate-700">{text || "Sem informações."}</p>
    </div>
  );
}
...
export default function SuprimentosTestes() {
  ...
  const filteredRows = useMemo(() => {
    return rows.filter((row) => {
      ...
      const searchOk = matchesSearch(row, search, ["nome_teste", "peca", "codigo_peca", "fornecedor", "prefixo", "aberto_por_nome"]);
  ...
  return (
    <div className="space-y-6">
      <PageHero
        eyebrow="Suprimentos"
        title="Testes"
        description="Acompanhe o comportamento da peça em campo, a quilometragem rodada e o parecer técnico até a decisão final de compra."
        actions={
          <>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
        ...
        <KpiCard title="Testes concluídos" value={cards.concluidos.length} subtitle="Com parecer e encerramento" icon={<FaFlagCheckered />} tone="emerald" />
        ...
        <KpiCard title="Reprovados" value={cards.reprovados.length} subtitle="Sem aderência técnica" icon={<FaTimesCircle />} tone="rose" />
        <KpiCard title="Com falha" value={cards.comFalha.length} subtitle="Falhas já registradas no teste" icon={<FaBug />} tone="violet" />
      ...
      <Panel
        ...
        subtitle="Use a busca para encontrar rapidamente a peça em teste, o prefixo e o responsável pela abertura."
      >
        <div className="grid gap-3 border-b border-slate-100 pb-4 lg:grid-cols-[1.2fr_0.45fr_0.25fr]">
          <label className="relative block">
            ...
            <input
              ...
              placeholder="Buscar teste, peça, fornecedor ou prefixo..."
              className={`${inputClass} pl-11`}
            />
          </label>

          <label className="block">
            <span className="mb-1.5 block text-xs font-black uppercase tracking-[0.2em] text-slate-500">Situação</span>
        ...
        {loading ? (
          ...
          <div className="mt-5">
            <EmptyState title="Nenhum teste encontrado" subtitle="Abra um teste novo para começar a acompanhar a peça em campo." />
          </div>
        ) : (
                    ...
                    return (
                      <tr key={row.id} className={`border-t border-slate-100 transition hover:bg-slate-50 ${active ? "bg-blue-50/60" : ""}`}>
                        ...
                        <td className="px-4 py-3">
                          ...
                          <p className="mt-1 text-xs text-slate-500">{kmRodado !== null ? `Rodado ${formatKm(kmRodado)}` : "Sem atualização"}</p>
                        </td>
                        <td className="px-4 py-3">
