...
rotulado as (
  select
    a.*,
    round(100 - ((a.total_com_problema_camera::numeric * 100) / nullif(a.total_laudos::numeric, 0)), 1) as acertividade,
    round((a.total_com_problema_camera::numeric * 100) / nullif(a.total_laudos::numeric, 0), 1) as taxa_problema,
    case
...
select
  ...
  (
    r.total_com_problema_camera >= greatest(ceil(r.total_laudos::numeric * 0.15)::bigint, 10)
    or r.total_tecnica >= 3
  ) as necessita_ajuste_camera
from rotulado r
where
  r.total_com_problema_camera >= greatest(ceil(r.total_laudos::numeric * 0.15)::bigint, 10)
  or r.total_tecnica >= 3
order by acertividade asc, prioridade_camera desc, total_com_problema_camera desc, prefixo;
